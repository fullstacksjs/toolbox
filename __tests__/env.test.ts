import { getEnv, getNodeEnv, getRequiredEnv, match, matchDev, matchProd, matchTest } from '../src/env';
...
describe('env', () => {
  ...
  describe('match', () => {
    it('should return false if env is different', () => {
      ...
      expect(match('development')).toBe(false);
    });

    it('should return true if env exact same', () => {
      process.env.NODE_ENV = 'production';
      expect(match('production')).toBe(true);
    });
