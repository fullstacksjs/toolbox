...
debounce returns a function which will call the callback function provided to
it, only if a certain delay is past since its last call. Useful for seachboxes,
buttons, etc.
...
const debounced = debounce({ delay: 1000 }, execute);
...
const debounced = debounce({ delay: 1000, immediate: true }, execute);
