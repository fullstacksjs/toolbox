...
const hasDepType =
  (packageJson: PackageJson) => (type: Dependencies) => (dep: string) =>
    typeof packageJson[type] === 'object' &&
...
export const hasAnyDep = (packageJson: PackageJson) => (dep: string) =>
  [hasDep, hasDevDep, hasPeerDep, hasOptDep].some(isDepExist =>
    isDepExist(packageJson)(dep),
  );

export const ifAnyDep =
  (packageJson: PackageJson) => (dep: string, t?: Function, f?: Function) =>
